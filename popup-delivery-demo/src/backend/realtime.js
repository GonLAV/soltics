...
// Clearing sessions without closing their sockets left every open customer
// tab in a state that looked healthy — socket still open, page still reading
// "connected" — while every event it sent 404'd. Close them, so the client
// sees the disconnect and re-handshakes.
function reset() {
  pending.clear();

  for (const [sessionId, ws] of sockets) {
    try {
      ws.close(4001, 'server state reset');
    } catch {
      /* already closing */
    }
  ...
  bus.trace('realtime', 'Closed all sockets for state reset', { liveConnections: sockets.size });
}
